...
const Menu = () => {
	...
	const ToolsButtons = [
		{
			name: "Zarządzanie warstwami",
			fnc: "LayersManagement",
			special: false,
			ico: <MdManageSearch className="icon" />,
			desc: `
      Narzędzie powstało w związku z ilością warstw tematycznych tak aby łatwiej było nimi zarządzać.
      Pełni ono również funkcję zwijanej legendy dla każdej z warstw. Aby ustawić warstwę ponad innymi należy ją złapać i przeciągnąć wyżej.
      `,
		},
		{
			name: "Mini mapa",
			fnc: "MiniMap",
			special: false,
			ico: <HiLocationMarker className="icon" />,
			desc: "Narzędzie powstało aby użytkownik mógł łatwiej zlokalizować swoje położenie na mapie. Wyświetla ono widok użytkownika względem całej mapy",
		},
		{
			name: "Pomiary",
			fnc: "",
			special: true,
			ico: <BsRulers className="icon" />,
			desc: "Narzędzie stworzone do mierzenia powierzchni i długości. Po jego włączeniu można rysować i modyfikować różnego rodzaju kształty.",
		},
		{
			name: "Rysuj panel",
			fnc: "DrawPanel",
			...
			desc: `
      Narzędzie stworzone aby dostarczyć więcej informacji na temat usłonecznienia dachu, posiada dwie wersje.
				...
				{ToolsButtons.map((el, index) => {
					...
					return (
						<button
							...
							key={index}
							onClick={() => {
								special ? RemoveDrawToolsHandler() : ToolToggle(fnc);
							}}
						>
							<span>
								{ico} {name}
