...
import { MapContainer, GeoJSON, Polygon, Marker } from "react-leaflet";
...
const MiniMap = () => {
	...
	const { mapBounds, removeTool, zoomLevel, mapCenter } =
		...
		<DraggableComponent nodeRef={DraggableRef}>
			<Container ref={DraggableRef}>
				...
				<MapContainer
					...
					dragging={false}
				>
					{polygonCoords != null && zoomLevel < 18 ? (
						<Polygon pathOptions={purpleOptions} positions={polygonCoords} />
					) : (
						<Marker icon={GetIcon()} position={mapCenter}></Marker>
					)}
